Paper_Fig_Project <- function(results, run_project=FALSE, start_sim, nproject=10, pres=FALSE, 
	...
	if(run_project==TRUE){
        ...
        for(mod in 1:ncol(b_est)){
        	...
        	b_proj_t[mod,1] <- b_true_start[1,mod] + ((z_true^(z_true/(z_true-1)))/(z_true-1))*msy_true[1,mod]*((b_true_start[1,mod]/K_true[1,mod])-(b_true_start[1,mod]/K_true[1,mod])^z_true) - c_true_start[1,mod]
            c_proj_t[mod,1] <- TAC[mod,1]/rep[mod]
            # b_proj_t[mod,1] <- max(1, b_true_start[1,mod] + ((z_true^(z_true/(z_true-1)))/(z_true-1))*msy_true[1,mod]*((b_true_start[1,mod]/K_true[1,mod])-(b_true_start[1,mod]/K_true[1,mod])^z_true) - c_true_start[1,mod])
            # c_proj_t[mod,1] <- min(b_proj_t[mod,1], TAC[mod,1]/rep[mod])
       		...
       		for(yr in 2:nproject){
                ...
                b_proj_t[mod,yr] <- b_proj_t[mod,yr-1] + ((z_true^(z_true/(z_true-1)))/(z_true-1))*msy_true[1,mod]*((b_proj_t[mod,yr-1]/K_true[1,mod])-(b_proj_t[mod,yr-1]/K_true[1,mod])^z_true) - c_proj_t[mod,yr-1]
                c_proj_t[mod,yr] <- TAC[mod,yr]/rep[mod]
                # b_proj_t[mod,yr] <- max(1, b_proj_t[mod,yr-1] + ((z_true^(z_true/(z_true-1)))/(z_true-1))*msy_true[1,mod]*((b_proj_t[mod,yr-1]/K_true[1,mod])-(b_proj_t[mod,yr-1]/K_true[1,mod])^z_true) - c_proj_t[mod,yr-1])
    ...
    if(pres==FALSE){
      ...
      #     polygon(x=c(1:35, 35:1), y=c(q_c_true[1,,i]/q_b_true[1,,i], rev(q_c_true[3,,i]/q_b_true[3,,i])), col="#0000FF50", border=NA)
            ...
            abline(h=msy_true[1, index[i]], lty=2)
            # abline(v=35, lty=2)
            if(i==1){
    ...
    if(pres==TRUE) par(mfrow=c(3,5), mar=c(0,0,0,0), omi=c(1,1,1,0.2))
        for(i in 1:length(index)){
    		...
    		abline(h=bmsy_true[1,index[i]], col="black", lty=2)
        ...
        for(i in 1:length(index)){
            ...
            lines(x=1:35, y=b_true[1,index[i],]/bmsy_true[1,index[i]], lwd=2, col=gray(0.2))
    	    lines(x=35:(35+nproject), y=c(b_true[1,index[i],35]/bmsy_true[1,index[i]], b_proj_t[index[i],]/bmsy_true[1,index[i]]), lwd=2, col=gray(0.2))
      ...
      #   	polygon(x=c(1:35, 35:1), y=c(q_c_true[1,,i]/q_b_true[1,,i], rev(q_c_true[3,,i]/q_b_true[3,,i])), col="#0000FF50", border=NA)
            ...
            lines(x=1:35, y=((c_true[1,index[i],]/b_true[1,index[i],])/(msy_true[1,index[i]]/bmsy_true[1,index[i]])), lwd=2, col=gray(0.2))
        	lines(x=35:(35+nproject), y=c((c_true[1,index[i],35]/b_true[1,index[i],35])/(msy_true[1,index[i]]/bmsy_true[1,index[i]]), (c_proj_t[index[i],]/b_proj_t[index[i],])/(msy_true[1,index[i]]/bmsy_true[1,index[i]])), col=gray(0.2), lwd=2)
